...
def main():
    ...
    for label in ALL_LABELS:
        ...
        for state in STATES:
            ...
            # --- overrides for known bad/missing source data ---
            if label == "coconut":
                ...
                price_val, price_src = np.nan, "national fallback (crop_benchmarks_MASTER.csv)"

            if label in HORTICULTURE_COST_INR_HA and not c:
                ...
                cost_val, cost_src = np.nan, "national fallback (crop_benchmarks_MASTER.csv)"

            row = {
                "label": label,
                "state": state,
                "avg_yield_t_ha": y["avg_yield_t_ha"] if y else np.nan,
                "yield_source": y["yield_source"] if y else "national fallback (crop_benchmarks_MASTER.csv)",
    ...
    # Fill missing yield/cost with a national fallback figure derived from
    # whatever real state rows DO exist for that crop, so no row is left blank.
    for col, source_col in [("avg_yield_t_ha", "yield_source"), ("cost_cultivation_inr_ha", "cost_source")]:
        fallback_avg = out_df.groupby("label")[col].transform(
            lambda s: s.mean() if s.notna().any() else np.nan
        )
        out_df[col] = out_df[col].fillna(fallback_avg)
