...
# Reject implausible single-source yields (e.g. a state with only one year of
# data ever recorded, producing a freak low/high number with nothing to
# cross-check it against) rather than trusting a single unverified data point.
MIN_PLAUSIBLE_YIELD_T_HA = {
    "coconut": 3.0,   # anything below this is almost certainly a bad single-year record
}
...
def build_state_yield(crop_yield_path="crop_yield.csv"):
    ...
    for label, apy_crop in YIELD_NAME_MAP.items():
        ...
        for state in STATES:
            ...
            # Plausibility check: reject a value below the known-sane floor when
            # it's backed by only one year of data (nothing to cross-verify against)
            floor = MIN_PLAUSIBLE_YIELD_T_HA.get(label)
            if floor is not None and yield_val < floor and n_years_available <= 1:
                continue  # skip -- falls through to the placeholder/override fallback later
...
def build_state_cost(cost_paths=("Cost_of_Cultivation_Sample_Data.csv",)):
    ...
    most recent year available per (state, crop). Pass multiple file paths to
    combine several extracts into one wider, de-duplicated real-data pool."""
