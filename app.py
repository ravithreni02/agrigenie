...
# Horticulture Statistics Division, DAC&FW (2019-20)
WATERMELON_STATES = {
...
# Coffee Board of India (2025-26 Final Estimate)
COFFEE_STATES = {
...
# 2023-24 horticulture trade reporting, cross-confirmed across sources
MUSKMELON_STATES = {
...
def get_coords(district):
    ...
    if coords is None:
        raise ValueError(f"No coordinates for {district}")
    return coords
...
def climate_risk_assessment(district):
    try:
        ...
        return {"district": district, "temperature": 27.0, "humidity": 65.0, "rainfall_37d_actual": 150.0, "rainfall_37d_normal": 150.0, "pct_departure": 0.0, "category": "Unknown", "risk": "low", "source": f"fallback ({e})"}
...
def multi_objective_recommend(state, district, N=None, P=None, K=None, ph=None, top_n=5, w_confidence=0.5, w_yield=0.05, w_water=0.4, w_profit=0.05):
    ...
    def get_shap(idx):
        return shap_v[idx][0] if isinstance(shap_v, list) else shap_v[0, :, idx]
    ...
    for c, cf in zip(cands, confs):
        ...
        d_idx = int(np.argmax(np.abs(s_row)))
        d_feat = FEATURES[d_idx]
        ...
        rows.append({
            ...
            "margin_flag": b_row.get("margin_flag", "ok"),
        })

    res = pd.DataFrame(rows)
    if res.empty:
        return res, soil, (N is None), clim, {"raw_top_crop": raw_top_crop, "raw_top_conf": raw_top_conf, "suppressed": False}
    ...
    if clim["risk"] != "low":
        res["s_water"] *= (0.7 if clim["risk"] == "moderate" else 0.4)
...
if st.sidebar.button("🌾 Recommend", type="primary", use_container_width=True):
    ...
    if fb:
        st.info(f"📋 Fallback soil for {sel_dist}: N={soil['N']}, P={soil['P']}, K={soil['K']}, pH={soil['ph']}")
    ...
    display_cols = ["crop", "classifier_confidence", "estimated_yield_t_ha", "irrigation_needed_mm", "estimated_profit_inr_ha", "margin_flag", "growth_status", "data_confidence", "xai_explanation"]
    st.dataframe(res[display_cols], use_container_width=True, hide_index=True)

    if (res["margin_flag"] == "low_margin_verified_real_data").any():
        st.caption("ℹ️ Rows flagged **low_margin_verified_real_data** stay unprofitable even under best-case local climate, using real state yield/cost/price data — not a calculation error, a genuine thin-margin result.")
